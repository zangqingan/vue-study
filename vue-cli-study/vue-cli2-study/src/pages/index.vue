    ...
    <!-- product -->
      <div class="index-left-block">
        ...
        <div v-for="product in productList" :key="product.id">
          <h3>{{product.title}}</h3>
          <ul>
            <li v-for="opitem in product.list" :key="opitem.id">
              <a :href="opitem.url">{{opitem.name}}</a>
              <!-- hot选项设置 -->
              <span v-if="opitem.hot" class="hot-tag">HOT</span>
            </li>
          </ul>
          <!-- 最后一项不渲染hr -->
...
export default {
    // 在vue声明周期组件创建完毕后
    // created () {
    //     this.$http.get('getList').then(data =>{
    //         console.log(data)
    //     },err =>{
    //         console.log(err)
    //     })
